    ...
    if (IsItemProbablyActive(item, mDisplayListBuilder)) {
      ...
      if (!groupData->mFollowingGroup.mGroupBounds.IsEqualEdges(currentGroup->mGroupBounds) ||
        ...
        if (groupData->mFollowingGroup.mKey) {
          LayerIntRect layerBounds = LayerIntRect::FromUnknownRect(mGroupBounds.ScaleToOutsidePixels(mScale.width, mScale.height, mAppUnitsPerDevPixel));
          groupData->mFollowingGroup.mInvalidRect = IntRect(IntPoint(0, 0), layerBounds.Size());
